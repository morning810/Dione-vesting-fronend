...
import { GolobalMainWeb3, getEndOfDay, getStartOfDay, getTransactionEvents, isValidEthereumAddress } from '../chain_interaction/common';
...
const Create = () => {
    ...
    const handleCreateContract = async () => {
        ...
        if (enableMultisign) {
            ...
            const found = signCandidators?.find(item => item.toString().toLowerCase() === address?.toLowerCase())
        ...
        if (isNative == 1) {
            ...
            if (Number(parseUnits(amount?.toString(), tokenDecimals)?.toString()) < 0.1) {
        ...
        try {
            ...
            let transactionArguments = [
                ...
                isNative == 1 ? parseEther(amount)?.toString() : parseUnits(amount?.toString(), tokenDecimals)?.toString(),
            ...
            if (enableMultisign == true) {
                transactionArguments = [...transactionArguments, signCandidators, Number(numConfirmationRequired)];
            }

            const createHash = await walletClient.writeContract({
                address: addressesOnActiveNetwork.VestingFactory,
                abi: VestingFactoryAbi,
                functionName: enableMultisign !== true ? "create" : "createWithMultisigWallet",
                args: transactionArguments,
            });

            const receipt = await waitForTransaction({ hash: createHash });

            const logs = await getTransactionEvents(GolobalMainWeb3, VestingFactoryAbi, receipt.logs);
            ...
            if (logs && logs?.length > 0) {
                ...
                if (owner?.toString().toLowerCase() === address?.toLowerCase()) {
                    ...
                    saveNewVesting({
                        ...
                        amount: isNative == 1 ? parseEther(amount)?.toString() : parseUnits(amount?.toString(), tokenDecimals)?.toString(),
    ...
    const readTokenInformation = async (tokenAddress, address) => {
        try {
            ...
            const tokenContract = new GolobalMainWeb3.eth.Contract(ERC20Abi, tokenAddress);
            let readingPromises = [];
            ...
            Promise.all(readingPromises).then(results => {
                if (address) {
                    ...
                    setTokenBalance(parseFloat(formatUnits(userBalance?.toString(), parseInt(results[0]?.toString()))));
                }
    ...
    return <div className="text-white mb-10 w-full md:w-auto flex flex-col items-center bg-[#19191920] border-[#343143] border-[1px] rounded-[50px] mt-28 md:mt-0">

        <div className='absolute -top-[250px] -right-[400px] flex w-full justify-start '>
            <div className="explore-pink-radial transform rotate-90  h-[860px] w-[860px] opacity-50 ">
            </div>
        </div>
        <div className='absolute top-0 bottom-0 left-0 right-0 flex justify-start items-center z-0 '>
            <div className="vesting-darkblue-radial  h-[600px] w-[600px] opacity-50 ">
            </div>
        </div>

        <div className='text-[26px] md:text-[24px] lg:text-[28px] font-semibold py-8' >Create a new {isNative == 1 ? "vesting" : "locker"}</div>
        <div className="w-[96vw] md:w-auto px-2 md:px-10 py-10  min-h-[450px] flex flex-col gap-5 items-center ">
            {
                isNative == 0 &&
                <>
                    <Accordion open={open === 0}>
                        <AccordionHeader className='text-white hover:text-white border-none outline-none'
                            onClick={() => handleOpen(0)}>
                            <div className='flex justify-start gap-2 items-center'><VscGroupByRefType className='w-[32px] h-[32px]' />Select Type</div>
                        </AccordionHeader>
                        <AccordionBody className="text-white" >
                            <div className="w-full flex justify-between px-5">
                                <div className="flex items-center ">
                                    <div className="text-[16px] font-semibold  min-w-[120px] ">Type:</div>
                                    <select className="border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] outline-none "
                                        value={type} onChange={(e) => setType(e.target.value)}
                                    >
                                        <option className="bg-[#12101c] text-white py-4 border-none" value={0} >Token locker</option>
                                        <option className="bg-[#12101c] text-white py-4 border-none" value={1} >Liquidity Locks</option>
                                    </select>
                                </div>
                            </div>
                        </AccordionBody>
                    ...
                    <Accordion open={open === 1}>
                        <AccordionHeader className='text-white hover:text-white border-none outline-none' onClick={() => handleOpen(1)}>
                            <div className='flex justify-start gap-2 items-center'><MdOutlineGeneratingTokens className='w-[32px] h-[32px]' />Enter {type == 0 ? "Token " : "LP Token "} Address</div>
                        </AccordionHeader>
                        <AccordionBody className="text-white w-full flex justify-center flex-col ">
                            <div className="flex items-center w-full px-2 md:px-16 ">
                                <div className="text-[16px] font-semibold min-w-[80px] flex justify-start ">{type == 0 ? "Token:" : "LP Token:"}</div>
                                <input className="border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12]  outline-none min-w-[calc(100%-120px)] "
                                    placeholder=""
                                    value={token}
                            ...
                            <div className="flex items-center w-full mt-5 px-2 md:px-16 ">
                                <div className="text-[16px] font-semibold min-w-[80px] flex justify-start ">Symbol: </div>
                                <div className="text-white px-5 py-2 bg-transparent outline-none min-w-[calc(100%-120px)] text-left "
                                >{tokenSymbol}</div>
                            </div>
                            <div className="flex items-center w-full mt-5 px-2 md:px-16 ">
                                <div className="text-[16px] font-semibold min-w-[80px] flex justify-start ">Balance: </div>
                                <div className="px-5 py-2 bg-transparent outline-none min-w-[calc(100%-120px)] text-left "
                                >{Number(tokenBalance).toFixed(4)}</div>
                            </div>
                            <div className="flex items-center w-full mt-5 px-2 md:px-16 ">
                                <div className="text-[16px] font-semibold min-w-[80px] flex justify-start ">Decimals: </div>
                                <div className="px-5 py-2 bg-transparent outline-none min-w-[calc(100%-120px)] text-left"
                                >{tokenDecimals}</div>
                            </div>
                        </AccordionBody>
            ...
            <Accordion open={isNative == 1 ? alwaysOpen : open == 2}>
                <AccordionHeader className='text-white hover:text-white border-none outline-none' onClick={() => { isNative == 0 && setOpen(2) }}>
                    <div className='flex justify-start gap-2 items-center'><BiMessageSquareDetail className='w-[32px] h-[32px]' />Add Details</div>
                </AccordionHeader>
                <AccordionBody className="flex flex-col gap-4 text-white w-full px-2 md:px-10">

                    <div className="text-[16px] font-semibold min-w-[120px] text-left ">Name:</div>
                    <input className="border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] outline-none min-w-[calc(100%-120px)] "
                        placeholder={`${isNative == 1 ? "Vesting " : "Locker "}Name`}
                        value={name}
                        onChange={(e) => setName(e.target.value)}
                    />
                    <div className="text-[16px] font-semibold min-w-[120px]  text-left ">Amount:</div>
                    <input className="border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] outline-none min-w-[calc(100%-120px)] "
                        placeholder={`Number of ${isNative == 1 ? "DIONE to vest" : type == 0 ? "token to lock" : "LP token to lock"}`}
                        type="number"
                        value={amount}
                        onChange={(e) => setAmount(e.target.value)}
                    />

                    <div className="text-[16px] font-semibold min-w-[120px]  text-left ">Vesting period:</div>

                    <div className="flex items-center w-full  ">
                        <input className="border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] outline-none min-w-[calc(100%-165px)] "
                            placeholder={`${isNative == 1 ? "Total time to vest" : "Total time to lock"}`}
                            type="number"
                            value={period}
                            onChange={(e) => setPeriod(e.target.value)}
                        />
                        <select className="custom-select border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] outline-none w-auto md:min-w-[160px] ml-[5px] h-[40px]"
                            value={periodTimeUnit} onChange={(e) => setPeriodTimeUnit(e.target.value)}
                        >
                            <option className="bg-[#12101c] text-white py-2 border-none" value={"second"} >Second</option>
                            <option className="bg-[#12101c] text-white py-2 border-none" value={"minute"} >Minute</option>
                            <option className="bg-[#12101c] text-white py-2 border-none" value={"hour"} >Hour</option>
                            <option className="bg-[#12101c] text-white py-2 border-none" value={"day"} >Day</option>
                            <option className="bg-[#12101c] text-white py-2 border-none" value={"month"} >Month</option>
                        </select>
                    </div>

                    <div className="text-[16px] font-semibold min-w-[120px]  text-left ">Vesting rate:</div>

                    <div className="flex items-center w-full  ">
                        <input className="border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] outline-none w-full  "
                            placeholder={`Percentage of ${isNative == 1 ? "DIONE " : type == 0 ? "token " : "LP token "}can withdraw in each epoch`}
                            typed="number"
                            min={0}
                            max={100}
                            value={rate}
                    ...
                    <div className="text-[16px] font-semibold min-w-[120px]  text-left ">Recipients:</div>
                    <textarea className="border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] outline-none min-w-[calc(100%-120px)] min-h-[100px]"
                    ...
                    <div className="flex items-center w-full md:gap-4  justify-between">
                        <div className="flex  flex-row md:flex-col md:gap-3">
                            <div className="text-[16px] font-semibold  min-w-[120px] flex justify-start ">Start Date:</div>
                            <DatePicker className=" border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] max-w-[220px] outline-none " selected={startDate} onChange={(date) => {
                        ...
                        <div className="hidden md:flex flex-col md:gap-3">
                            <div className="text-[16px] font-semibold min-w-[120px] flex justify-start  ">End Date:</div>
                            <DatePicker className=" border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] max-w-[220px] outline-none " selected={endDate} onChange={(date) => setEndDate(date)} />
                    ...
                    <div className="flex md:hidden items-center  ">
                        <div className="text-[16px] font-semibold  min-w-[120px]  text-left ">End Date:</div>
                        <DatePicker className=" border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] max-w-[220px] outline-none " selected={endDate} onChange={(date) => setEndDate(date)} />
                        {/* 
                        <DateTimePicker className=" border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] max-w-[220px] outline-none " selected={endDate} onChange={(date) => {
            ...
            <Accordion open={open === 3}>
                <AccordionHeader className='text-white hover:text-white border-none outline-none' onClick={() => handleOpen(3)}>
                    <div className='flex justify-start gap-2 items-center'><LiaSignatureSolid className='w-[32px] h-[32px]' />Set multi sig</div>
                </AccordionHeader>
                <AccordionBody className="text-white">
                    <div className="flex items-center w-full px-2 md:px-10 text-[16px] font-semibold">
                        Do you need to apply multiple wallet signatures to activate a deposit or withdrawal?
                    </div>
                    ...
                    <div className="text-[16px] font-semibold min-w-[70px] py-5 pl-10 text-left">Signers:</div>
                    <textarea className="border-[#343143] border rounded-xl px-5 py-2 bg-[#ffffff12] outline-none min-w-[calc(100%-70px)] min-h-[100px]"
                        placeholder="0x5dF66A3bD72dFC6c58321a52965edcE16bfCbbB1,0xDbfd0fdcfeC90B826Bd92142BbB44773584513A5"
                    ...
                    <div className='flex justify-center md:justify-start w-full mt-5 px-2 md:px-10 text-[16px] font-semibold items-center gap-2 ' >
                        <div className='max-w-[200px] md:min-w-[300px] text-wrap'>Number of confirmations required: </div>
                        <input
            ...
            <div className="w-full flex justify-center md:pr-8 z-10">
                <button className="bg-[#bf9aff] rounded-full px-4 py-3  
