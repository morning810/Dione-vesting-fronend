...
import { GolobalMainWeb3, VestingFactoryContract, copyToClipBoard, formatTime, getEndOfDay, getMinEthreumAddress, getStartOfDay, getTransactionEvents, isValidEthereumAddress } from '../chain_interaction/common';
...
import { activeChainId, addressesOnActiveNetwork } from '../chain_interaction/deployedAddresses';
...
import { getSingleVestingByAddress, readClaimesByPage, saveNewClaim } from '../backend_interaction';
...
const Vesting = () => {
    ...
    const [multiSignLastTxConfirmStatus, setMultiSignLastTxConfirmStatus] = useState([]);
    const [multiSignIsLastTxConfimed, setMultiSignIsLastTxConfimed] = useState(true);
    ...
    const columns = [
            ...
            renderCell: (params) => {
                return (
                    <div className="w-full pl-5 ">
                        <a href={`${addressesOnActiveNetwork?.blackScanUrl}address/${params?.value}`} target="_blank" className='text-white z-10'
                        >
        ...
        },
        {
            ...
            renderCell: (params) => {
                return (
                    <div className="flex gap-2">
                        <a href={`${addressesOnActiveNetwork.blackScanUrl}tx/${params?.value}`} target="_blank" className='text-white z-10' >
    ...
    const getTimeRemained = (startDate, endDate) => {
        ...
        const remaindRate = parseFloat(timeDifference?.toString()) * 100 / (endTime.getTime() - startTime.getTime());
    ...
    function getNextVestingDate(startDate, endDate, vestingPeriod) {
        ...
        while (nextVestingDate <= now) {
            nextVestingDate = new Date(nextVestingDate.getTime() + vestingPeriod * 1000);
        }
    ...
    const readVestingPublicParams = async (vesting_id) => {
        try {
            if (vesting_id && Number(vesting_id) >= 0) {
                let publicParams = await VestingFactoryContract.methods.readSpecificVesting(vesting_id).call();
                const paramsFromDB = await getSingleVestingByAddress(publicParams?.target);

                const amountTobeVested = parseFloat(formatUnits(publicParams?.amount, parseInt(publicParams?.decimals)));
                const abi = publicParams?.token !== addressesOnActiveNetwork.ZeroAddress ? tokenVestingAbi : nativeVestingAbi;
                const vestingContract = new GolobalMainWeb3.eth.Contract(abi, publicParams?.target);
                ...
                console.log(" publicParams?.start >> ", publicParams?.start, " publicParams?.end >> ", publicParams?.end)
                setStartTime(new Date(parseInt(publicParams?.start?.toString()) * 1000));
                setEndTime(new Date(parseInt(publicParams?.end?.toString()) * 1000));
                setAmountTobeVested(amountTobeVested);

                const totalVeAmount = await vestingContract.methods.vestedAmount().call();
                const multiSignAddress = paramsFromDB?.multiSignAddress;
                const owner = await vestingContract.methods.owner().call();
                let candidators = [], numConfirm = 0, lastTxId = 0, lastTxObj, lastTxConfimStatus, isTxConfirmed = true, isMultiSStarted = false;
                if (multiSignAddress !== addressesOnActiveNetwork?.ZeroAddress && isValidEthereumAddress(multiSignAddress)) {
                    try {
                        //read multi sign candidators and minimum number of confirmation
                        const multiSignCotract = new GolobalMainWeb3.eth.Contract(multiSignAbi, multiSignAddress);
                        ...
                        if (isMultiSStarted === true) {
                            ...
                            lastTxObj = await multiSignCotract.methods.transactions(lastTxId).call();
                            console.log("lastTxObj >>> ", lastTxObj);
                            lastTxConfimStatus = [];
                            isTxConfirmed = await multiSignCotract.methods.isTransactionConfirmed(lastTxId).call();
                            console.log("isTxConfirmed >>> ", isTxConfirmed);
                            for (let index = 0; index < candidators?.length; index++) {
                                try {
                                    let confirmed = await multiSignCotract.methods.isSignerConfirmed(lastTxId, candidators[index]).call();
                                    lastTxConfimStatus = [
                                        ...lastTxConfimStatus,
                                        {
                ...
                if (publicParams?.token !== addressesOnActiveNetwork.ZeroAddress) {
                    // read token balance
                    const tokenContract = new GolobalMainWeb3.eth.Contract(ERC20Abi, publicParams?.token);
                    contractBal = await tokenContract?.methods.balanceOf(publicParams?.target).call();
                    contractBal = parseFloat(formatUnits(contractBal, parseInt(publicParams?.decimals?.toString())));
                } else {
                    // read native balance
                    contractBal = await GolobalMainWeb3?.eth.getBalance(publicParams?.target);
                    contractBal = parseFloat(formatEther(contractBal));
                }
                if (address) {
                    const myVeAmount = await vestingContract.methods.released(address).call();
                    setMyReleasedAmount(parseFloat(formatUnits(myVeAmount?.toString(), parseInt(publicParams?.decimals?.toString()))));
                }
                setTotalVestedAmount(parseFloat(formatUnits(totalVeAmount, parseInt(publicParams?.decimals?.toString()))));
        ...
        try {
            ...
            if (address && contractAddress) {
                ...
                const claimHash = await walletClient.writeContract({
                    ...
                    abi: vestingToken !== addressesOnActiveNetwork.ZeroAddress ? tokenVestingAbi : nativeVestingAbi,
                    functionName: "claim",
                    args: [
                    ],
                });
                ...
                const logs = await getTransactionEvents(GolobalMainWeb3, vestingToken !== addressesOnActiveNetwork.ZeroAddress ? tokenVestingAbi : nativeVestingAbi, receipt.logs);
                ...
                if (logs && logs?.length > 0) {
                    ...
                    if (newReleasedToRecipientLog?.name === "ReleasedToRecipient") {
                        ...
                        saveNewClaim({
                            ...
                            amount: parseFloat(formatUnits(newReleasedToRecipientLog?._amount, parseInt(tokenDecimals?.toString()))),
    ...
    const readLiveHistory = async (pagenationmodel) => {
        ...
        const claims = await readClaimesByPage(pagenationmodel.page, pagenationmodel.pageSize, vesting_id);
    ...
    const prepareMultiSignTx = async () => {
        try {
            ...
            const logs = await getTransactionEvents(GolobalMainWeb3, multiSignAbi, receipt.logs);
            if (logs && logs?.length > 0) {
    ...
    const confirmMulisign = async (signerAddress) => {
        try {
            ...
            if (signerAddress?.toString()?.toLowerCase() !== address?.toLowerCase()) return;
            setWorking(true);
    ...
    const executeMultiSignTx = async () => {
        try {
            ...
            multiSignLastTxConfirmStatus?.map((item, index) => {
                if (item?.confimration === true) numberOfCurrentConfirms = Number(numberOfCurrentConfirms) + Number(1);
            });
            if (numberOfCurrentConfirms < multiSignMinNumOfCofirm) {
                toast.warn("Can not excute transaction before number of confirmation exceed the required min number of confirmation.");
                return;
            }
            setWorking(true);
            let amountToDeposit = amountTobeVested - contractBalance > 0 ? amountTobeVested - contractBalance : 0;
            if (vestingToken !== addressesOnActiveNetwork.ZeroAddress) {
                let amountToDepositWei = parseUnits(amountToDeposit?.toString(), tokenDecimals);
                const tokenContract = new GolobalMainWeb3.eth.Contract(ERC20Abi, vestingToken);
                let approvalsBefore = await tokenContract.methods.allowance(address, multiSignContractAddress).call();
                approvalsBefore = formatUnits(approvalsBefore?.toString(), tokenDecimals);
                const needApprove = parseFloat(approvalsBefore?.toString()) < parseFloat(amountToDeposit?.toString()) && amountToDeposit > 0;
                if (needApprove === true) {
                    const approveHash = await walletClient.writeContract({
                        ...
                        args: [
                            multiSignContractAddress,
                            amountToDepositWei
                        ],
                    });
                    const receipt = await waitForTransaction({ hash: approveHash });
                    const logs = await getTransactionEvents(GolobalMainWeb3, ERC20Abi, receipt.logs);
                    if (logs && logs?.length > 0) {
                        let newApproveLog = logs[0];
                        if (newApproveLog?.name === "Approval") {
                            toast.success("You've approved tokens successfully. We will continue excution.")
                        }
                    } else {
                        toast.error("Failed!");
                        setWorking(false);
    ...
    const depositRemainderFunds = async () => {
        try {
            ...
            if (contractBalance >= amountTobeVested) {
                ...
                if (vestingToken !== addressesOnActiveNetwork.ZeroAddress) {
                    amountToDeposit = parseUnits(amountToDeposit?.toString(), tokenDecimals);
                    setWorking(true);
    ...
    return <div className="text-white pb-20 pt-28 md:pt-0">
        <div className="flex items-center flex flex-col border border-panel-border-gray rounded-[50px] bg-[#19191920] relative">
            <div className='text-[26px] md:text-[24px] lg:text-[28px] font-semibold py-8' >{vestingToken !== addressesOnActiveNetwork?.ZeroAddress ? "Locker" : "Vesting"}  #{Number(vesting_id) + 1}</div>

            <div className='absolute -top-[25vw] -left-[15vw] md:-top-[250px] md:-left-[200px] flex w-full justify-start z-0'>
                <div className="explore-pink-radial transform -rotate-90 w-[100vw] h-[100vw] md:h-[860px] md:w-[860px] opacity-50 ">
                </div>
            </div>
            <div className='absolute top-0 bottom-0 left-0 right-0 flex justify-center items-center z-0 '>
                <div className="vesting-darkblue-radial  h-[600px] w-[600px] opacity-60 ">
                </div>
            </div>
                ...
                <div className=" flex flex-col items-center w-[96vw] md:ml-20 md:mx-0  md:w-auto">
                    <div className=" min-h-[350px] flex flex-col gap-5  md:w-full py-10 md:w-[40vw] justify-center">
                        <div className="w-full md:px-10 flex justify-between gap-9 md:gap-0">
                            <div className="w-6/12 text-week-white flex justify-start">{vestingToken !== addressesOnActiveNetwork?.ZeroAddress ? "Locker" : "Vesting"} Name</div>
                            <div className="w-6/12 text-[18px] font-semibold flex justify-start">{vestingName}</div>
                        </div>
                        <div className="w-full md:px-10 flex justify-between gap-8 md:gap-0">
                            <div className="flex flex-col w-6/12 items-start">
                                ...
                                <div className=" text-[18px] font-semibold z-10 flex gap-1 items-center">
                                    <a href={`${addressesOnActiveNetwork?.blackScanUrl}address/${contractAddress}`} target="_blank" className='text-white z-10'>{getMinEthreumAddress(contractAddress)}</a>
                                    <FaRegCopy className='cursor-pointer hover:transform hover:scale-110' onClick={() => copyToClipboard(contractAddress)} />
                                </div>
                            </div>
                            <div className="flex flex-col w-6/12 items-start">
                                ...
                                <div className=" text-[18px] font-semibold z-10 ">
                                    {
                                        vestingToken !== addressesOnActiveNetwork?.ZeroAddress ?
                                            <div className="flex gap-1 items-center" >
                                                <a href={`${addressesOnActiveNetwork?.blackScanUrl}address/${vestingToken}`} target="_blank" className='text-white z-10'>{getMinEthreumAddress(vestingToken)}</a>
                                                <FaRegCopy className='cursor-pointer hover:transform hover:scale-110' onClick={() => copyToClipboard(vestingToken)} />
                                            </div>
                                            :
                                            "Native"
                                    }
                        ...
                        <div className="w-full md:px-10 flex justify-between gap-8 md:gap-0">
                            <div className="flex flex-col w-6/12 items-start">
                                ...
                                <div className=" text-[18px] font-semibold z-10 flex gap-1 items-center">
                                    <a href={`${addressesOnActiveNetwork?.blackScanUrl}address/${sender}`} target="_blank" className='text-white z-10'>{getMinEthreumAddress(sender)}</a>
                                    <FaRegCopy className='cursor-pointer hover:transform hover:scale-110' onClick={() => copyToClipboard(sender)} />
                                </div>
                            </div>
                            <div className="flex flex-col w-6/12 items-start">
                                <div className=" text-week-white ">{receipentsCount > 1 ? "Recipients" : "Recipient"}</div>
                                <div className=" text-[18px] font-semibold z-10">
                                    {
                                        receipentsCount > 1 ?
                                            ...
                                            :
                                            <div className='flex gap-1 items-center'>
                                                <a href={`https://testnet.odysseyscan.com/${receipentWallets[0]}`} target="_blank" className='text-white'>{getMinEthreumAddress(receipentWallets[0])}</a>
                                                <FaRegCopy className='cursor-pointer hover:transform hover:scale-110' onClick={() => copyToClipboard(receipentWallets[0])} />
                                            </div>
                                    }
                                </div>
                            </div>
                        ...
                        <div className="w-full md:px-10 flex justify-between gap-14 md:gap-0">
                            <div className="flex flex-col w-6/12 items-start">
                                <div className=" text-week-white max-w-[100px] text-left">Expected Payout</div>
                                <div className=" text-[18px] font-semibold">{amountTobeVested?.toFixed(2)}</div>
                            </div>
                            <div className="flex flex-col w-6/12 items-start">
                                <div className=" text-week-white max-w-[100px] text-left">Contract Balance</div>
                                <div className=" text-[18px] font-semibold">{contractBalance?.toFixed(2)}</div>
                            </div>
                        </div>
                        <div className="w-full md:px-10 flex justify-between gap-14 md:gap-0">
                            <div className="flex flex-col w-6/12 items-start">
                                <div className=" text-week-white ">Rate</div>
                                <div className=" text-[18px] font-semibold">{vestingRate}%</div>
                            </div>
                            <div className="flex flex-col w-6/12 items-start">
                                <div className=" text-week-white ">Period</div>
                                <div className=" text-[18px] font-semibold">{formatTime(vestingPeriod)}</div>
                            </div>
                        </div>
                        <div className="w-full md:px-10 flex justify-between gap-14 md:gap-0">
                            <div className="flex flex-col w-6/12 items-start">
                                <div className=" text-week-white ">Revokable</div>
                                <div className=" text-[18px] font-semibold">{revokeAble ? "Yes" : "No"}</div>
                            </div>
                            <div className="flex flex-col w-6/12 items-start">
                        ...
                        <div className="w-full md:px-10 flex justify-between gap-14 md:gap-0">
                            <div className="flex flex-col w-6/12 items-start">
                                ...
                                <div className=" text-[18px] font-semibold">{startTime?.toLocaleDateString()}</div>
                            </div>
                            <div className="flex flex-col w-6/12 items-start">
                                <div className=" text-week-white ">End Date</div>
                                <div className=" text-[18px] font-semibold">{endTime?.toLocaleDateString()}</div>
                            </div>
                        </div>
                    </div>
                    <div className="w-10/12 md:w-[36vw]  flex justify-between  
                    ">
                        <div className="text-week-white">{vestingToken !== addressesOnActiveNetwork?.ZeroAddress ? "Locked" : "Vested"} Amount</div>

                        <div>
                            {isNaN(Number(totalVestedAmount * 100 / amountTobeVested)) ? 0 : Number(totalVestedAmount * 100 / amountTobeVested)?.toFixed(2)} %
                        </div>
                    ...
                    <div className="w-10/12 md:w-[36vw] mt-1 bg-[#12101C] border-[#343143] border-2 h-[24px] rounded-full relative">
                        <div className={`absolute top-0 left-0 bottom-0 rounded-full `}
                            style={{
                                ...
                                backgroundColor: `rgba(255, 64, 154, ${totalVestedAmount / amountTobeVested})`
                            }}
                        ></div>
                    </div>
                </div>
                <div className="w-[100vw] md:w-auto md:col-span-1 flex flex-col gap-5 justify-center  items-center pt-5 md:border-l-[2px] md:border-l-[#ffffff6a]">
                    <div className="w-[96vw]  md:w-9/12 flex justify-center items-center relative ">
                        ...
                        <div style={{ width: 300, height: 300 }}>
                            <CircularProgressbar
                                ...
                                styles={buildStyles({
                                    ...
                                    // pathTransition: 'none',
                                    // Colors
                                    pathColor: `rgba(255, 64, 154, ${getTimeRemained(startTime, endTime).remaindRate})`,
                            ...
                            <div className='absolute z-0 top-0 left-0 right-0 bottom-0 w-full h-full flex justify-center items-center'>
                                <div className='relative w-[300px] h-[300px] '>
                                    <img src="/vesting/Ellipse_outer.svg" className='w-full h-full' />
                                    <div className='absolute z-0 top-0 left-0 right-0 bottom-0 w-full h-full flex justify-center items-center'>
                                                ...
                                                <div className='relative w-[222px] h-[222px] '>
                                                    ...
                                                    <div className='absolute z-0 top-0 left-0 right-0 bottom-0 w-full h-full flex justify-center items-center'>
                                                        <div className='relative w-[120px] h-[120px] '>
                                                            <img src="/vesting/logo.svg" className='w-full h-full' />
                        ...
                        <div className='absolute top-0 left-0 right-0 bottom-0 flex text-center text-[24px] justify-center items-center z-5' >
                            <div className='bg-[#ffffff24] border-white h-[80px] w-[90vw] md:w-[400px] md:mx-0  rounded-2xl text-center flex items-center justify-center
                                text-[30px] font-medium
                            '>
                                {`${getTimeRemained(startTime, endTime).days}D ${getTimeRemained(startTime, endTime).hours}H ${getTimeRemained(startTime, endTime).minutes}M ${getTimeRemained(startTime, endTime).seconds}S`}
                            </div>
                        </div>
                    </div>
                    <div className="z-10">
                        <button className={` bg-[#ffffff12] rounded-full px-4 py-3 
                                text-white w-[200px] disabled:opacity-60
                            `}
                            onClick={() => claminMyVesting()}
                            disabled={!address || receipentWallets?.includes(address) === false}
                        >Claim</button>
                    </div>
                    <div className='flex flex-col gap-2 text-[16px]'>
                        <div className='text-week-white'>My claimed amount: <span className='text-white'>{Number(myReleasedAmount)?.toFixed(4)}</span></div>
                        <div className='text-week-white'>Next period starts: <span className='text-white'>{getNextVestingDate(startTime, endTime, vestingPeriod)}</span></div>
                    </div>
                </div>
            </div>
        </div>
        {
            isConnected && (multiSignCandidators.findIndex(item => item?.toString().toLowerCase() === address?.toLowerCase()) >= 0 || address?.toLowerCase() === vestingOwner?.toLowerCase()) &&
            <div className='w-full border border-panel-border-gray rounded-[50px] bg-[#19191920] min-h-[350px] flex flex-col md:gap-5  py-10 justify-center mt-5 items-center relative '>
                ...
                <div className='absolute top-0 -bottom-[300px] left-0 right-0 flex justify-end items-center z-0 '>
                    <div className="vesting-darkblue-radial  h-[900px] w-[900px] opacity-50 ">
                    </div>
                </div>
                <div className="w-full justify-start text-[24px] font-semibold  ml-5 px-5 text-left">
                    Manage {vestingToken !== addressesOnActiveNetwork?.ZeroAddress ? "Locker" : "Vesting"}
                </div>
                <div className="w-[96vw] mt-10 md:mt-0  md:w-11/12 flex md:border-b md:border-b-gray-700 pb-5" >
                    <div className='text-[20px] font-semibolc flex flex-col w-8/12 items-center z-10'>
                        ...
                        <div className="flex items-center gap-2  ">
                            <a className='text-white text-wrap' href={`${addressesOnActiveNetwork?.blackScanUrl}address/${contractAddress}`} target='_blank' >{getMinEthreumAddress(contractAddress)}</a>
                        ...
                        <div className="text-[14px] text-week-white" >
                            {
                                contractBalance < amountTobeVested ?
                                    <>Please deposit more funds to this vesting contract.<br /> Contract balance is less than expected amount {amountTobeVested} .<br />
                ...
                {
                    (multiSignContractAddress?.toLowerCase() === addressesOnActiveNetwork.ZeroAddress && vestingOwner?.toLowerCase() === address?.toLowerCase()) ?
                        <div className='flex justify-center gap-5 mt-2'>
                            ...
                            `}
                                    onClick={() => revokeFunds()}
                                    disabled={endTime?.getTime() > new Date()?.getTime() || contractBalance <= 0}
                                >Revoke</button>
                            </div>
                            {
                                contractBalance < amountTobeVested &&
                        ...
                        <div className='flex flex-col md:gap-3 w-10/12 items-center my-2' >
                            <div className="w-full flex justify-center gap-5 items-center ">
                                {
                                    ...
                                    <div className="flex items-center z-10">
                                        <div className="text-[16px] font-semibold  min-w-[60px] text-week-white ">Type:</div>
                                        <select className="border-[#343143] border rounded-xl px-5 py-2 bg-transparent outline-none "
                                            value={multiSignType} onChange={(e) => setMultiSignType(e.target.value)}
                                            disabled={(isEmpty(multiSignLastTxObj) == false && multiSignLastTxObj?.executed !== true)}
                                        >
                                            <option className="bg-[#ffffff12] text-black py-2 border-none" value={TransactionType.Fund} >Deposit</option>
                                            <option className="bg-[#ffffff12] text-black py-2 border-none" value={TransactionType.Revoke} >Revoke</option>
                                        </select>
                                    </div>
                                }
                                <div className="text-week-white">
                                    Minimum confirmation counts: <span className="text-white"> {multiSignMinNumOfCofirm}</span>
                                </div>
                            </div>
                            {
                                (vestingOwner?.toLowerCase() === address?.toLowerCase()
                            ...
                            `}
                                    ...
                                    disabled={(isEmpty(multiSignLastTxObj) == false && multiSignLastTxObj?.executed !== true)}
                                >Prepare multi sign</button>

                            }
                            {
                                multiSignLastTxConfirmStatus && multiSignLastTxConfirmStatus?.length > 0 &&
                                multiSignLastTxObj?.executed === false &&
                                <div className='w-full max-h-[400px] flex flex-col items-center overflow-h-auto mt-3 z-10'>
                                    {
                                        multiSignLastTxConfirmStatus?.map((item, index) => (
                                            (item?.signer.toString()?.toLowerCase() !== vestingOwner?.toLowerCase()) ?

                                                <div className='flex w-full flex-col items-center md:flex-row justify-between gap-5 py-2' key={index} >
                                                    <div className='w-9/12 md:hidden'>{getMinEthreumAddress(item?.signer)}</div>
                                                    <div className='w-9/12 hidden md:block '>{item?.signer}</div>
                                                    ...
                                                    <div className='w-3/12 flex justify-center md:justify-start '>
                                                        {
                                                            item?.confimration === true ?
                                                                ...
                                                                :
                                                                <button className='bg-[#ffffff12] text-white  rounded-full z-10 '
        ...
        <div className='w-full overflow-none border border-panel-border-gray rounded-[50px] bg-[#19191920] min-h-[350px] flex flex-col gap-5 mt-5 py-10 justify-center relative '>
            ...
            <div className='absolute -top-[150px] -right-[800px] flex w-full justify-start '>
                <div className="explore-pink-radial transform -rotate-90  h-[860px] w-[860px] opacity-30 ">
                </div>
            </div>
        ...
        <div className='overflow-none w-full border border-panel-border-gray rounded-[50px] bg-[#19191920] min-h-[400px] flex flex-col gap-5  py-10 justify-center my-5 relative'>
            ...
            <div className='absolute top-0 bottom-0 left-0 right-0 flex justify-center items-center  '>
                <div className="vesting-darkblue-radial  h-[800px] w-[800px] opacity-50 ">
                </div>
            </div>
